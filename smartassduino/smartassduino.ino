...
int fsr_pin[N_SENSORS] = {A0, A1, A2, A3};

int fsr[N_SENSORS];
float score[N_SENSORS];
int timer_in_ms[N_SENSORS];
int delay_for_loop = 100;
int delay_pressure_relief = 5000;
int delay_until_warning = 10000;

int timer_lift = 0;

int delay_until_lift = 15 * 60 * 1000;

int fsrReading = 0;

char publishBuffer[ 512 ];
unsigned long publishInterval = 750;
...
//  delay(500);
  ...
  for(int i=0; i<N_SENSORS; i++) {
        digitalWrite( fsr_pin[i], HIGH ); //enable pullup resistor
    }
}
...
//  }
 ...
 sprintf( publishBuffer, "{\"flex1\": %d, \"flex2\": %d, \"flex3\": %d, \"flex4\": %d}", 
  ...
  ble.print(publishBuffer);
  Serial.print("\n");
  Serial.print(publishBuffer);
